...
public partial class 课体表
{
    ...
    private sealed class 六壬辨疑卷二提供类
    {
        ...
        private IReadOnlyList<四课之一> 取四课()
        {
            ...
            {
                this.四课 = new 四课之一[4] {
                        new 四课之一(this.传入四课.日, this.传入四课.日阳),
                        new 四课之一(this.传入四课.日阳, this.传入四课.日阴),
                        new 四课之一(this.传入四课.辰, this.传入四课.辰阳),
                        new 四课之一(this.传入四课.辰阳, this.传入四课.辰阴),
                    };
            }
            return this.四课;
        }
        ...
        {
            if (this.不重复四课 is null)
            {
                this.不重复四课 = this.取四课().DistinctBy(课 => 课.上).ToArray();
            }
            return this.不重复四课;
        }
        private (IReadOnlyList<四课之一> 课, bool 为贼) 取克课()
        {
            if (!this.克课.HasValue)
            {
                ...
                this.克课 = (有贼否 ? 下贼上课 : 上克下课, 有贼否);
            }
            return this.克课.Value;
        }
        ...
        private (bool 遵循顺逆, bool 为顺) 取天将顺逆()
        {
            if (!this.天将顺逆.HasValue)
            {
                ...
                IEnumerable<int> 取实际()
                {
                    ...
                    {
                        yield return (int)this.传入天将盘.取乘将(new(序号));
                    }
                }
                ...
                if (实际.SequenceEqual(顺行预期(起)))
                    ...
                    this.天将顺逆 = (true, false);
                else
                    this.天将顺逆 = (false, true);
            }
            return this.天将顺逆.Value;
        }
        private IEnumerable<Dizhi> 迭代三传()
        {
            yield return this.传入三传.初传;
            yield return this.传入三传.中传;
            yield return this.传入三传.末传;
        }
        private IEnumerable<Dizhi> 迭代四课(bool 含日 = true)
        {
            if (含日)
                yield return this.传入四课.日.Jigong();
            yield return this.传入四课.日阳;
            yield return this.传入四课.日阴;
            yield return this.传入四课.辰;
            yield return this.传入四课.辰阳;
            yield return this.传入四课.辰阴;
        }
        #region 卷二
        [判课体法]
        public bool 元首()
        ...
        public bool 遥克()
        {
            ...
            if (克课.Count is not 0)
                return false;
            return this.取遥克课().Count is not 0;
        }
